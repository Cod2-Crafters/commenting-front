...
import { APIResponseMsg, ConversationAnswerWriteSchemaState, ConversationQuestionLikeItResponse, ConversationQuestionLikeItSchemaState, ConversationQuestionWriteSchemaState, ConversationSchemaState } from '@/schemas'
...
export const toggleQuestionLikeIt = createAsyncThunk<APIResponseMsg<ConversationQuestionLikeItResponse>, ConversationQuestionLikeItSchemaState>('items/toggleQuestionLikeIt', async (formData, { getState, rejectWithValue }) => {
  try {
    ...
    return { ...response.data, number: formData.conId }
  } catch (error) {
    return rejectWithValue(error.response?.data || 'An error occurred')
  }
})
...
// 좋아요 등록 및 제거
// toggleConversationLikeIt: (state, action: PayloadAction<ConversationQuestionLikeItResponse>) => {
...
export const { setConversations, appendConversation } = conversationSlice.actions
