...
const StatisticContent = ({ ...props }: StatisticContentProps) => {
  ...
  const fetchStatistics = async () => {
    const response = await axiosClient.get<APIResponseMsg<StatisticInfo>>('/api/statistics', {
      headers: {
  ...
  const statisticsQuery = useQuery({ queryKey: ['statistics', authSelector.token], queryFn: fetchStatistics })
            ...
            <div className="flex flex-col gap-8">
              ...
              <div className="flex flex-col gap-8">
                <div className="flex flex-row justify-between">
                  ...
                  <div className="text-xl font-semibold">{statisticsQuery.data.data.goodQuestionCount}</div>
                </div>
                <div className="flex flex-row justify-between">
                  <h4 className="text-xl font-semibold">내가 받은 질문</h4>
                  <div className="text-xl font-semibold">{statisticsQuery.data.data.receivedQuestionCount}</div>
                </div>
                <div className="flex flex-row justify-between">
                  <h4 className="text-xl font-semibold">다른 사람에게 작성한 질문</h4>
                  <div className="text-xl font-semibold">{statisticsQuery.data.data.sentQuestionCount}</div>
                </div>
                <div className="flex flex-row justify-between">
                  <h4 className="text-xl font-semibold">작성한 답변</h4>
                  <div className="text-xl font-semibold">{statisticsQuery.data.data.answerCount}</div>
                </div>
                <div className="flex flex-row justify-between">
                  <h4 className="text-xl font-semibold">답변하지 않은 질문</h4>
                  <div className="text-xl font-semibold">{statisticsQuery.data.data.unansweredQuestionCount}</div>
                </div>
              </div>
